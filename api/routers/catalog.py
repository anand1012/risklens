...
def get_asset(asset_id: str):
    sql = f"""
        ...
        FROM `{project()}.risklens_catalog.assets_s` a
        LEFT JOIN `{project()}.risklens_catalog.ownership_s` o USING (asset_id)
        LEFT JOIN `{project()}.risklens_catalog.quality_scores_s` q USING (asset_id)
        LEFT JOIN `{project()}.risklens_catalog.sla_status_s` s USING (asset_id)
...
def get_asset_schema(asset_id: str):
    sql = f"""
        ...
        FROM `{project()}.risklens_catalog.schema_registry_s`
